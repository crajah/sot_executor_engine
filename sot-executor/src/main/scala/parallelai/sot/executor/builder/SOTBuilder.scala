...
object SOTBuilder {
  ...
  class Job extends Serializable {
    def execute(sotUtils: SOTUtils, sc: ScioContext, args: Args): Unit = {
      val job = init[HNil].flatMap(sColls => read(sc, TapDef[parallelai.sot.executor.model.SOTMacroConfig.KafkaTapDefinition, parallelai.sot.engine.config.gcp.SOTUtils, parallelai.sot.engine.io.utils.annotations.HasJSONAnnotation, saf](conf.sourceTaps(0)._3), sotUtils)).flatMap(sColls => writeSchemaless1(sColls.at(Nat0()))(SchemalessTapDef[parallelai.sot.executor.model.SOTMacroConfig.BigQueryTapDefinition, parallelai.sot.engine.config.gcp.SOTUtils, parallelai.sot.engine.io.utils.annotations.Schemaless](conf.sinkTaps(0)._3), sotUtils))
      job.run(HNil)._1
      val result = sc.close()

...
trait ToMappableBQ[L <: HList, M] extends Serializable {
  def apply(l: L): M
}

trait LowPriorityToMappable1 {
  implicit def hconsToMappable1[K <: Symbol, V, T <: HList, M]
  (implicit wit: Witness.Aux[K], mt: MappableTypeBQ[M, V], toT: Lazy[ToMappableBQ[T, M]])
  : ToMappableBQ[FieldType[K, V] :: T, M] = new ToMappableBQ[FieldType[K, V] :: T, M] {
...
trait LowPriorityToMappableOption1 extends LowPriorityToMappable1 {
  ...
  (implicit wit: Witness.Aux[K], mt: MappableTypeBQ[M, V], toT: Lazy[ToMappableBQ[T, M]])
  : ToMappableBQ[FieldType[K, Option[V]] :: T, M] = new ToMappableBQ[FieldType[K, Option[V]] :: T, M] {
...
trait LowPriorityToMappableSeq1 extends LowPriorityToMappableOption1 {
  ...
  (implicit wit: Witness.Aux[K], mt: MappableTypeBQ[M, V], toT: Lazy[ToMappableBQ[T, M]],
   toSeq: S[V] => Seq[V])
  : ToMappableBQ[FieldType[K, S[V]] :: T, M] = new ToMappableBQ[FieldType[K, S[V]] :: T, M] {
...
trait LowPriorityToMappable0 extends LowPriorityToMappableSeq1 {
  ...
  (implicit wit: Witness.Aux[K], gen: LabelledGeneric.Aux[V, H], mbt: BaseMappableTypeBQ[M],
...
object ToMappableBQ extends LowPriorityToMappableSeq0 {
  implicit def hnilToMappable[M](implicit mbt: BaseMappableTypeBQ[M])
  : ToMappableBQ[HNil, M] = new ToMappableBQ[HNil, M] {

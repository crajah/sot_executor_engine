...
object DeepRec extends LowPriorityDeepRec {
  ...
  implicit def hconsDeepRec2[K <: Symbol, V, Repr <: HList, T <: HList](implicit
                                                                        gen: LabelledGeneric.Aux[V, Repr],
                                                                        hdr: Lazy[DeepRec[Repr]],
                                                                        tdr: Lazy[DeepRec[T]]
    ...
    new DeepRec[FieldType[K, Option[V]] :: T] {
      ...
      def apply(
                 in: FieldType[K, Option[V]] :: T
               ): FieldType[K, Option[hdr.value.Out]] :: tdr.value.Out = {
      ...
      def fromRec(
                   out: FieldType[K, Option[hdr.value.Out]] :: tdr.value.Out
  ...
  implicit def hconsDeepRec3[K <: Symbol, V, Repr <: HList, T <: HList](implicit
                                                                        gen: LabelledGeneric.Aux[V, Repr],
                                                                        hdr: Lazy[DeepRec[Repr]],
                                                                        tdr: Lazy[DeepRec[T]]
    ...
    new DeepRec[FieldType[K, List[V]] :: T] {
      ...
      def apply(
                 in: FieldType[K, List[V]] :: T
               ): FieldType[K, List[hdr.value.Out]] :: tdr.value.Out =
        field[K](in.head.map(v => hdr.value(gen.to(v)))) :: tdr.value(in.tail)
      def fromRec(
                   out: FieldType[K, List[hdr.value.Out]] :: tdr.value.Out
                 ): FieldType[K, List[V]] :: T =
